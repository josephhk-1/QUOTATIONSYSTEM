...
const companyProfiles = {
    ...
    'rattan_palace': { 
        acronym: 'RP', logo: 'https://placehold.co/256x100/f59e0b/FFFFFF?text=Rattan+Palace',
...
// ===============================================
// 2. FIREBASE DATA FUNCTIONS
// ===============================================

function listenForQuotes() {
    if (!currentUser) return;
    if (unsubscribeQuotes) unsubscribeQuotes();
    unsubscribeQuotes = db.collection('users').doc(currentUser.uid).collection('quotes')
      .orderBy('meta.savedAt', 'desc')
      .onSnapshot(snapshot => {
        savedQuotes = snapshot.docs.map(doc => ({ ...doc.data(), meta: { ...doc.data().meta, id: doc.id } }));
...
async function saveQuoteToFirestore(quoteData) {
    if (!currentUser) return;
    const quoteRef = db.collection('users').doc(currentUser.uid).collection('quotes');
    try {
        if (currentlyEditingQuoteId) {
            await quoteRef.doc(currentlyEditingQuoteId).set(quoteData, { merge: true });
        } else {
            const docRef = await quoteRef.add(quoteData);
            currentlyEditingQuoteId = docRef.id;
        }
        alert('Quotation saved!');
        showDashboard();
    } catch (error) { console.error("Error saving quote: ", error); alert("Failed to save."); }
}
...
async function deleteQuote(quoteId) {
    ...
    if (confirm('Are you sure you want to delete this quotation?')) {
        try {
            await db.collection('users').doc(currentUser.uid).collection('quotes').doc(quoteId).delete();
        } catch (error) { console.error("Error deleting quote: ", error); alert("Failed to delete."); }
    }
}
...
function listenForProducts() {
    ...
    unsubscribeProducts = db.collection('users').doc(currentUser.uid).collection('products').onSnapshot(snapshot => {
        products = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
        if (!document.getElementById('editor-page').classList.contains('hidden')) renderProducts();
    });
}
async function addProduct() {
    ...
    if ((desc_en || desc_ar) && !isNaN(price)) {
        await db.collection('users').doc(currentUser.uid).collection('products').add({ desc_en, desc_ar, price });
...
function listenForClients() {
    ...
    unsubscribeClients = db.collection('users').doc(currentUser.uid).collection('clients').onSnapshot(snapshot => {
        clients = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
        if (!document.getElementById('editor-page').classList.contains('hidden')) renderClients();
    });
}
async function addClient() {
    ...
    if (name_en || name_ar) {
        await db.collection('users').doc(currentUser.uid).collection('clients').add({ name_en, name_ar, address_en, address_ar });
...
async function deleteClient(clientId) {
    if (!currentUser) return;
    await db.collection('users').doc(currentUser.uid).collection('clients').doc(clientId).delete();
}
...
function saveCurrentQuote() {
    ...
    saveQuoteToFirestore(quoteData);
}
...
function captureQuoteData() {
    ...
    const data = {
        ...
        fields: {}, items: [],
        meta: { savedAt: firebase.firestore.FieldValue.serverTimestamp() },
    ...
    document.querySelectorAll('#table-body tr').forEach(row => {
        ...
        data.items.push({
            photo: img.src.startsWith('https://') ? img.src : '',
...
function applyQuoteData(data) {
    ...
    const today = data.meta && data.meta.savedAt ? new Date(data.meta.savedAt.seconds * 1000) : new Date();
...
function addNewRow(item = { photo: '', desc_en: '', desc_ar: '', qty: 1, price: 0 }) {
    ...
    row.querySelector('.load-product-btn').addEventListener('click', () => {
        openModal('Select a Product', products, (p) => `<strong>${p.desc_en || p.desc_ar}</strong> - Price: ${p.price}`, (product) => {
            const desc = row.querySelector('.item-description');
            desc.dataset.en = product.desc_en; desc.dataset.ar = product.desc_ar;
...
function renderProducts() {
    ...
    products.forEach(p => {
        ...
        item.innerHTML = `<span>${p.desc_en || p.desc_ar} - <strong>${p.price}</strong></span><button class="delete-btn" data-id="${p.id}">×</button>`;
...
function renderClients() {
    ...
    clients.forEach(c => {
        const item = document.createElement('div');
        item.className = 'list-item text-sm';
        item.innerHTML = `<span>${c.name_en || c.name_ar}</span><button class="delete-btn" data-id="${c.id}">×</button>`;
        item.querySelector('.delete-btn').onclick = () => deleteClient(c.id);
        list.appendChild(item);
    });
}
...
async function generatePDF() {
    ...
    clone.style.fontFamily = (currentLang === 'ar') ? 'Cairo, sans-serif' : 'Inter, sans-serif';
    ...
    try {
        const canvas = await html2canvas(clone, { scale: 3, useCORS: true, allowTaint: true });
        ...
        if (user) {
            ...
            auth.signInAnonymously().catch(error => {
                ...
                if (error.code === 'auth/configuration-not-found') {
                    alert('Firebase Authentication Error: Anonymous sign-in is not enabled. Please go to your Firebase Console -> Authentication -> Sign-in method and enable the "Anonymous" provider.');
                } else {
